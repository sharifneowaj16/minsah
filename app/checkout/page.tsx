...
function CheckoutContent() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      // addressId: numeric id = local/temp id, skip it; real CUID = use it
      const isRealDbId =
        selectedAddress.id && !/^\d+$/.test(selectedAddress.id);
      ...
      const res = await fetch('/api/orders', {
        ...
        credentials: 'include',
        body: JSON.stringify({
          // ── FIX: use item.productId (not item.id which can be variantId) ──
